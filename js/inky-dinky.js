...
let info =
  "An inky dinky is two words, like INKY and DINKY. Each word of the inky dinky has two syllables, like INKY and DINKY. The first syllable of the first word rhymes with the first syllable of the second, and vice versa, like INKY AND DINKY. Good luck!";
...
function generateInkyDinky() {
  ...
  document.getElementById("inky-dinky").innerHTML =
    currentInkyDinky.description;
...
function submitGuess() {
  ...
  if (
    currentInkyDinky.answer
      .toLowerCase()
      .trim()
      .indexOf(guess.toLowerCase().trim()) >= 0 ||
    guess
      .toLowerCase()
      .trim()
      .indexOf(currentInkyDinky.answer.toLowerCase().trim()) >= 0
  ) {
    showCorrectBanner();
  } else {
    showIncorrectBanner();
...
function showIncorrectBanner() {
  document.getElementsByClassName("correct-banner")[0].style.display = "none";
  document.getElementsByClassName("incorrect-banner")[0].style.display =
    "block";
}
